...
def test_controlled_batch_rolls_back_if_later_item_drifts(tmp_path):
    for index in range(2):
        path = tmp_path / "workflows" / f"{index}.json"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(_workflow(f"Fixture {index}")), encoding="utf-8")
    ...
    with psycopg.connect(os.environ["N8N_BRAIN_TEST_DATABASE_URL"]) as connection:
        ...
        original_second = compiled[1].records
        write_prepared_records(connection, original_second)

        drifted_second = copy.deepcopy(original_second)
        drifted_second.knowledge["structure"]["name_untrusted"] = "Drifted"

        with pytest.raises(ImmutableDriftError):
            write_prepared_records_batch(
                connection,
                [compiled[0].records, drifted_second],
            )
